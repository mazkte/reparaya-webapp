...
import { Reporte, ReporteFilter, EstadoReporteEnum, CategoriaEnum, PrioridadEnum } from '../../shared/models';
...
@Component({
  ...
  imports: [
    ...
    MatChipsModule, MatTooltipModule, MatProgressSpinnerModule, MatProgressBarModule
  ],
  template: `
    <div class="report-list">
      ...
      <mat-card class="filter-card">
        ...
        @if (tieneFilters()) {
          <div class="active-filters">
            ...
            @if (searchCtrl.value) {
              <mat-chip (removed)="searchCtrl.setValue('')">
                "{{ searchCtrl.value }}"
                <button matChipRemove><mat-icon>cancel</mat-icon></button>
              </mat-chip>
            }
      ...
      <mat-card class="table-card">
        @if (loading() && reportes().length === 0) {
          ...
          <table mat-table [dataSource]="reportes()" class="w-full">
            <ng-container matColumnDef="id">
              <th mat-header-cell *matHeaderCellDef>#</th>
              <td mat-cell *matCellDef="let r"><span class="report-id">#{{ r.id }}</span></td>
            </ng-container>
            ...
            <ng-container matColumnDef="categoria">
              ...
              <td mat-cell *matCellDef="let r">
                <span class="chip" [ngClass]="'cat-' + r.categoria">{{ getLabelCategoria(r.categoria) }}</span>
              </td>
            </ng-container>
            ...
            <ng-container matColumnDef="empresa">
              ...
              <td mat-cell *matCellDef="let r">
                @if (r.empresaNombre) {
                  <span class="empresa-badge"><mat-icon class="emp-icon">business</mat-icon>{{ r.empresaNombre }}</span>
                } @else {
            ...
            <ng-container matColumnDef="fecha">
              ...
              <td mat-cell *matCellDef="let r"><span class="fecha">{{ formatDate(r.fechaCreacion) }}</span></td>
            </ng-container>
            ...
            <tr mat-row *matRowDef="let row; columns: displayedColumns;"
                class="report-row" [routerLink]="['/reportes', row.id]"></tr>
  ...
  styles: [`
    ...
    .w-full { width: 100%; }
    .loading-center { display: flex; flex-direction: column; align-items: center; gap: 12px; padding: 48px; color: #6b6b8a; font-size: 13px; }
    ...
    .report-loc { display: flex; align-items: center; gap: 2px; font-size: 11px; color: #6b6b8a; }
    ...
    .no-data { text-align: center; padding: 48px !important; color: #6b6b8a;
      mat-icon { font-size: 40px; width: 40px; height: 40px; color: #d0cfe8; display: block; margin: 0 auto 12px; }
      p { margin: 0 0 16px; }
    }
...
export class ReportListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.searchCtrl.valueChanges.pipe(debounceTime(500), distinctUntilChanged())
      .subscribe(() => { this.currentPage = 0; this.cargarReportes(); });

    [this.estadoCtrl, this.categoriaCtrl, this.prioridadCtrl].forEach(ctrl =>
      ctrl.valueChanges.subscribe(() => { this.currentPage = 0; this.cargarReportes(); })
    );
  }
  ...
  onPage(e: PageEvent) { this.currentPage = e.pageIndex; this.pageSize = e.pageSize; this.cargarReportes(); }

  limpiarFiltros() {
    this.estadoCtrl.setValue(''); this.categoriaCtrl.setValue('');
    this.prioridadCtrl.setValue(''); this.searchCtrl.setValue('');
  }

  getLabelEstado(e: string)    { return this.estados.find(x => x.value === e)?.label ?? e; }
  getLabelCategoria(c: string) { return this.categorias.find(x => x.value === c)?.label ?? c; }
